...
export const StudentBatchToolbar: React.FC<StudentBatchToolbarProps> = ({
  ...
  return (
    <AnimatePresence>
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 max-w-2xl w-[92%] sm:w-auto pointer-events-auto">
        <motion.div
          initial={{ opacity: 0, y: 40, scale: 0.95 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          exit={{ opacity: 0, y: 40, scale: 0.95 }}
          transition={{ duration: 0.25, ease: 'easeOut' }}
          className="flex flex-wrap items-center justify-between gap-3 p-3 sm:px-5 sm:py-3.5 rounded-3xl bg-slate-900/95 dark:bg-slate-900/95 border border-slate-700 shadow-2xl text-white backdrop-blur-xl"
        >
          {/* Left Selection Info */}
